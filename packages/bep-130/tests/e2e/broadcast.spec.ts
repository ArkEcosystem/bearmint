...
extendExpect(expect)
import { BEP126Collection, BEP126MsgOp, BEP129MsgOp, BEP130MsgOp } from '@bearmint/bep-018'
...
import { makeServiceProvider as BEP130 } from '../../source/service-provider.js'
...
describe<{
	...
	beforeEach(async (ctx) => {
		...
		ctx.bep130 = await E2E.createEnvironment(BEP130)
	})
	...
	it('should pass', async (ctx) => {
		...
		// 2. Grant Permissions
		const grant = await ctx.bep129.txFactory.asserted(
		...
		// 3. Revoke BURN permission
		const revokeBurn = await ctx.bep130.txFactory.asserted(
			ctx.bep130.accts.get('genesis_1').keypair,
			ctx.bep130.accts.get('genesis_1').keypair,
			await ctx.bep130.client.fakeTxMsg('@bearmint/bep-130').execute({
				ops: [
					new BEP130MsgOp({
						hash: collectionHash,
						permissions: [BEP126UserPermission.BURN],
						steward: ctx.bep130.accts.get('genesis_2').address,
					}),
				],
			}),
		)

		await revokeBurn.pass()
		await revokeBurn.confirmMetaMsg(
			collectionHash,
			{
				accessControlList: {
					[ctx.bep130.accts.get('genesis_1').address]: {
						permissions: [0, 1, 2],
					},
					[ctx.bep130.accts.get('genesis_2').address]: {
						permissions: [1],
						role: 1,
					},
				},
				currentOwner: ctx.bep130.accts.get('genesis_1').address,
				id: collectionHash,
				initialOwner: ctx.bep130.accts.get('genesis_1').address,
				name: collection.name,
				policies: [
					{
						name: MintPolicy.ONLY_TEAM,
		...
		// 4. Revoke MINT permission
		const revoke = await ctx.bep130.txFactory.asserted(
			ctx.bep130.accts.get('genesis_1').keypair,
			ctx.bep130.accts.get('genesis_1').keypair,
			await ctx.bep130.client.fakeTxMsg('@bearmint/bep-130').execute({
				ops: [
	...
	E2E.testLoopback('@bearmint/bep-130', it, 'bep130')
	E2E.testMemoLength('@bearmint/bep-130', it, 'bep130')
	E2E.testOpCount('@bearmint/bep-130', it, 'bep130')

	it('should fail if it there are duplicate ops by hash and steward', async (ctx) => {
		const tx = await ctx.bep130.txFactory.asserted(
			ctx.bep130.accts.get('genesis_1').keypair,
			ctx.bep130.accts.get('genesis_1').keypair,
			await ctx.bep130.client.fakeTxMsg('@bearmint/bep-130').execute({
				ops: [
		...
		await tx.fail([
			{
				...
				type: '@bearmint/bep-130/0.0.0/audit',
			},
		])
	})
	...
	it('should fail if it the sender does not own the collection', async (ctx) => {
		...
		const tx = await ctx.bep130.txFactory.asserted(
			ctx.bep130.accts.get('genesis_2').keypair,
			ctx.bep130.accts.get('genesis_2').keypair,
			await ctx.bep130.client.fakeTxMsg('@bearmint/bep-130').execute({
				ops: [
					new BEP130MsgOp({
			...
			await ctx.bep130.client.fakeTxMsg('@bearmint/bep-130').execute({
				ops: [
					new BEP130MsgOp({
						hash: collectionHash,
						permissions: [BEP126UserPermission.BURN, BEP126UserPermission.MINT],
						steward: ctx.bep130.accts.get('genesis_1').address,
					}),
				],
			}),
		)
	...
	it('should fail if the steward does not have the permission', async (ctx) => {
		const { collectionHash } = await makeCollection(ctx.bep126, ctx.bep126.accts.get('genesis_1'))

		await (
			await ctx.bep130.txFactory.asserted(
				ctx.bep130.accts.get('genesis_1').keypair,
				ctx.bep130.accts.get('genesis_1').keypair,
				await ctx.bep130.client.fakeTxMsg('@bearmint/bep-130').execute({
					ops: [
