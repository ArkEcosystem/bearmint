...
export function makeServiceProvider(cradle: Cradle): ServiceProvider {
	return {
		...
		async register() {
			cradle.StrategyRegistry.set('@bearmint/bep-102', '@bearmint/bep-121', makeStrategy)

			// State
			await cradle.CommittedState.getMultiStore().add('@bearmint/bep-121')
			...
			// Prevent stake increases for tombstoned validators
