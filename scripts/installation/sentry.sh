...
read -p "Would you like to configure the Validator Peer ID? [y/N]: " choice

if [[ "$choice" =~ ^(yes|y|Y) ]]; then
	validator=""
	while [[ -z "$sentries" ]]; do
		validator=$(readNonempty "Enter the Peer ID (id@ip:port) of your validator: ")
	done

	node $HOME/bearmint/packages/bep-117/bin/index.mjs config preset sentry --validator="$validator"
else
	node $HOME/bearmint/packages/bep-117/bin/index.mjs config preset sentry
fi
