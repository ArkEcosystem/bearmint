...
read -p "Would you like to configure the Sentry Peer IDs? [y/N]: " choice

if [[ "$choice" =~ ^(yes|y|Y) ]]; then
	sentries=""
	while [[ -z "$sentries" ]]; do
		sentries=$(readNonempty "Enter the Peer IDs (id@ip:port) of your sentries: ")
	done

	node $HOME/bearmint/packages/bep-117/bin/index.mjs config preset validator --sentries="$sentries"
else
	node $HOME/bearmint/packages/bep-117/bin/index.mjs config preset validator
fi
