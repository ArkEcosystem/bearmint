...
extendExpect(expect)
import { BEP126MsgOp, BEP127MsgOp } from '@bearmint/bep-018'
...
import { makeServiceProvider as BEP127 } from '../../source/service-provider.js'
...
describe<{
	...
	beforeEach(async (ctx) => {
		...
		ctx.bep127 = await E2E.createEnvironment(BEP127)
	})

	it('should pass', async (ctx) => {
		const collectionHash = await makeCollection(ctx.bep126, ctx.bep126.accts.get('genesis_1'))

		const tx = await ctx.bep127.txFactory.asserted(
			ctx.bep127.accts.get('genesis_1').keypair,
			ctx.bep127.accts.get('genesis_1').keypair,
			await ctx.bep127.client.fakeTxMsg('@bearmint/bep-127').execute({
				ops: [
					new BEP127MsgOp({
						hash: collectionHash,
						recipient: ctx.bep127.accts.get('genesis_2').address,
					}),
				],
			}),
		)

		await ctx.bep127.accts
			.get('genesis_1')
		...
		ctx.bep127.accts.assertExpectations()
	})

	E2E.testLoopback('@bearmint/bep-127', it, 'bep127')
	E2E.testMemoLength('@bearmint/bep-127', it, 'bep127')
	E2E.testOpCount('@bearmint/bep-127', it, 'bep127')
		...
		await tx.fail([
			{
				...
				type: '@bearmint/bep-127/0.0.0/audit',
			},
		])
	})

	it('should fail if it there are any malformed recipient addresses', async (ctx) => {
		const collectionHash = await makeCollection(ctx.bep126, ctx.bep126.accts.get('genesis_1'))
	...
	it('should fail if it the sender does not own the collection', async (ctx) => {
		...
		const tx = await ctx.bep127.txFactory.asserted(
			ctx.bep127.accts.get('genesis_2').keypair,
			ctx.bep127.accts.get('genesis_2').keypair,
			await ctx.bep127.client.fakeTxMsg('@bearmint/bep-127').execute({
				ops: [
					new BEP127MsgOp({
