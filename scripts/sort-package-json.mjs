// Based on https://github.com/keithamus/sort-package-json
...
const sortObjectBy = (comparator, deep) => {
	if (comparator === undefined) {
		comparator = (a, b) => a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' })
	}
...
const sortScripts = onObject((scripts, packageJson) => {
	...
	const order = keys.reduce(
		(order, key) => order.concat(prefixable.has(key) ? [`pre${key}`, key, `post${key}`] : [key]),
		[],
	)
...
;(await globby(process.argv.slice(2))).forEach((file) => {
