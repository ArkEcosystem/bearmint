...
import { BEP126Collection } from '@bearmint/bep-018'
...
export class Query implements QueryHandler {
	...
	async isMatch({ data, path }: QueryHandlerOpts) {
		...
		return path[0] === 'store' && path[1] === '@bearmint' && path[2] === 'bep-126'
	}
	...
	async execute({ data }: QueryHandlerOpts) {
		...
		try {
			...
			return {
				code: 0,
				value: jsonToBytes(BEP126Collection.fromBinary(value, PROTO_OPTS).toJson()),
			}
		}
	}
}
