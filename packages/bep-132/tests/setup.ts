...
export async function createTx(
	...
	return {
		...
		tx: await context.makeTx(
			ownedByBuyer === true ? recipient.keypair : sender.keypair,
			ownedByBuyer === true ? recipient.keypair : sender.keypair,
			await context.client.fakeTxMsg('@bearmint/bep-132').execute({
				...
				ops: payload?.ops ?? [
					{
						...
						recipient: ownedByBuyer === true ? sender.account.address : recipient.account.address,
					},
				],
			}),
		),
