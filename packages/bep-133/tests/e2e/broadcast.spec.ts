...
import { BEP126MsgOp, BEP129MsgOp, BEP131MsgOp } from '@bearmint/bep-018'
...
import { makeServiceProvider as BEP133 } from '../../source/service-provider.js'
...
describe<{
	...
	beforeEach(async (ctx) => {
		...
		ctx.bep133 = await E2E.createEnvironment(BEP133)
	})

	it('should burn the token as the owner', async (ctx) => {
		...
		const tx = await ctx.bep133.txFactory.asserted(
			ctx.bep133.accts.get('genesis_1').keypair,
			ctx.bep133.accts.get('genesis_1').keypair,
			await ctx.bep133.client.fakeTxMsg('@bearmint/bep-133').execute({
				ops: [{ hash: tokenHash }],
			}),
		)

		await ctx.bep133.accts
			.get('genesis_1')
			.expectModuleMetadataNotContains('@bearmint/bep-131', tokenHash)

		await tx.pass()

		ctx.bep133.accts.assertExpectations()
	})

	it('should burn the token as a steward', async (ctx) => {
		...
		const tx = await ctx.bep133.txFactory.asserted(
			ctx.bep133.accts.get('genesis_2').keypair,
			ctx.bep133.accts.get('genesis_2').keypair,
			await ctx.bep133.client.fakeTxMsg('@bearmint/bep-133').execute({
				ops: [{ hash: tokenHash }],
			}),
	...
	E2E.testLoopback('@bearmint/bep-133', it, 'bep133')
	E2E.testMemoLength('@bearmint/bep-133', it, 'bep133')
	E2E.testOpCount('@bearmint/bep-133', it, 'bep133')
		...
		await tx.fail([
			{
				...
				type: '@bearmint/bep-133/0.0.0/audit',
			},
		])
	})
	...
	it('should fail to burn tokens from an immutable collection', async (ctx) => {
		...
		await (
			await ctx.bep133.txFactory.asserted(
				ctx.bep133.accts.get('genesis_1').keypair,
				ctx.bep133.accts.get('genesis_1').keypair,
				await ctx.bep133.client.fakeTxMsg('@bearmint/bep-133').execute({
					ops: [{ hash: tokenHash }],
	...
	it('should fail to burn tokens from a height-locked collection if the final height has been passed', async (ctx) => {
	...
	it('should fail to burn tokens from a max-supply-locked collection if the max supply has been reached', async (ctx) => {
	...
	it('should fail to burn tokens from a collection as a steward while only the owner is allowed to burn (overrules ACL)', async (ctx) => {
		const { collectionHash } = await makeOnlyOwnerCollection(
			ctx.bep126,
			ctx.bep126.accts.get('genesis_1'),
		)
	...
	it('should fail to burn tokens from a collection where only the team can burn but the sender is not part of the team', async (ctx) => {
		const { collectionHash } = await makeOnlyTeamCollection(
			ctx.bep126,
			ctx.bep126.accts.get('genesis_1'),
		)
