 ...
 * service metadata.
 */
function createGrpcMethodDefinition<T extends ServiceType, M extends keyof T['methods']>(
	service: T,
	methodLocalName: M,
	...
	return {
		...
		requestStream:
			info.kind === MethodKind.ClientStreaming || info.kind === MethodKind.BiDiStreaming,
		...
		responseStream:
			info.kind === MethodKind.ServerStreaming || info.kind === MethodKind.BiDiStreaming,
	}
}
...
export function createGrpcServiceDefinition<T extends ServiceType>(
	...
	for (const localName of Object.keys(service.methods)) {
		def[localName] = createGrpcMethodDefinition(service, localName, binaryOptions)
	}
	return def
}
