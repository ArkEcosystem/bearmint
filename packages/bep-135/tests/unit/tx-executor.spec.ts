...
describe<TxTestContext>('Tx Executor', () => {
	...
	it('should transfer the native token to the recipient and remove it from the sender', async (context) => {
		...
		expect(
			await context.state.getMultiStore().get('@bearmint/bep-134').getListHex(recipient.address),
		).toStrictEqual([])
		...
		expect(
			...
			BEP134Token.fromJson({
				currentOwner: recipient.address,
